...
const createSignupSchema = (t: any) =>
  zod
    .object({
      ...
      confirmPassword: zod.string().min(6, t('validation.minLength', { min: 6 })),
    })
...
type SignupFormValues = zod.infer<ReturnType<typeof createSignupSchema>>;

function SignupFormContent({ locale }: { locale: string }) {
  ...
  const onSubmit = (values: SignupFormValues) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        toast.success(t('auth.successProfileUpdate') || 'Account created successfully!');
        ...
        <CardContent className="space-y-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-1.5 text-start">
              <Label htmlFor="username" className="text-xs font-bold text-foreground/80 uppercase">
                {t('auth.username')}
              </Label>
              <Input
                id="username"
            ...
            <div className="space-y-1.5 text-start">
              <Label htmlFor="confirmPassword" className="text-xs font-bold text-foreground/80 uppercase">
              ...
              {errors.confirmPassword && (
                <p className="text-xs font-semibold text-destructive mt-1">{errors.confirmPassword.message}</p>
              )}
            </div>
...
// Reuse LoginFormContent for styling/rendering registration form in local context
const LoginFormContent = SignupFormContent;
