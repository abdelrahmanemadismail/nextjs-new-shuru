...
export async function generateMetadata({ params }: GenerateMetadataProps) {
  ...
  return buildMetadata({
    locale,
    path: "/insights/majlis",
    title: t("tabs.majlis") || "Majlis",
...
export default async function Page({ params, searchParams }: Props) {
  ...
  const { data: majlises, meta } = await getMajlisPaginatedCached(locale, current_page, 9, searchQuery, sortOrder);
  const t = await getTranslations({ locale, namespace: 'insights' });
    ...
    <main className="container py-24 mx-auto px-4 max-w-7xl">
      ...
      {majlises.length === 0 ? (
        <div className="text-center py-12 border border-dashed rounded-xl bg-muted/10">
          <p className="text-muted-foreground">{locale === 'ar' ? 'لم يتم العثور على جلسات مجلس.' : 'No majlis sessions found.'}</p>
        </div>
      ) : (
        <>
